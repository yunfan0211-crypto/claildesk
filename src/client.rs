...
impl Client {
    ...
    async fn _start(peer: &str, conn_type: ConnType) -> ResultType<(Stream, bool)> {
        ...
        for i in 1..=3 {
            ...
            use claildesk_common::protobuf::ProtobufEnum;
    ...
    async fn request_relay(
        ...
        for i in 1..=3 {
            // use different socket due to current  implement requiring different nat address for each attempt
            let mut socket =
...
impl AudioHandler {
    ...
    fn start_audio(&mut self, format0: AudioFormat) -> ResultType<()> {
        ...
        use claildesk_common::config::APP_NAME;
...
impl LoginConfigHandler {
    ...
    pub fn handle_peer_info(&mut self, username: String, pi: PeerInfo) {
        if !pi.version.is_empty() {
            self.version = claildesk_common::get_version_number(&pi.version);
        }
