...
fn main() {
    ...
    {
        use claildesk_common::env_logger::*;
        init_from_env(Env::default().filter_or(DEFAULT_FILTER_ENV, "info"));
    }
    #[cfg(feature = "inline")]
    {
        let mut path = claildesk_common::config::Config::log_path();
    ...
    if args.is_empty() {
        ...
        #[cfg(windows)]
        {
            if args[0] == "--uninstall" {
                ...
                claildesk_common::allow_err!(platform::update_me());
                return;
            } else if args[0] == "--reinstall" {
                claildesk_common::allow_err!(platform::uninstall_me());
                claildesk_common::allow_err!(platform::install_me("desktopicon startmenu",));
                return;
            }
        }
        if args[0] == "--remove" {
            ...
            if args.len() == 2 {
                claildesk_common::config::Config::import(&args[1]);
            }
            return;
...
fn main() {
    ...
    let matches = App::new("claildesk")
        ...
        .author("CarrieZ Studio<info@claildesk.com>")
