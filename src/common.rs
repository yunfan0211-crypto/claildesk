...
use claildesk_common::{config::RENDEZVOUS_PORT, futures::future::join_all};
...
async fn test_rendezvous_server_() {
    ...
    claildesk_common::config::ONLINE.lock().unwrap().clear();
...
#[tokio::main(flavor = "current_thread")]
async fn _check_software_update() -> claildesk_common::ResultType<()> {
    sleep(3.).await;
    ...
    use claildesk_common::protobuf::Message;
    if let Some(Ok((bytes, _))) = socket.next_timeout(30_000).await {
        if let Ok(msg_in) = RendezvousMessage::parse_from_bytes(&bytes) {
            if let Some(rendezvous_message::Union::software_update(su)) = msg_in.union {
                let version = claildesk_common::get_version_from_url(&su.url);
...
pub fn get_full_name() -> String {
    format!(
        "{}.{}",
        claildesk_common::config::ORG,
        claildesk_common::config::APP_NAME,
    )
}

pub fn is_ip(id: &str) -> bool {
    claildesk_common::regex::Regex::new(r"^\d+\.\d+\.\d+\.\d+(:\d+)?$")
        .unwrap()
        .is_match(id)
}
