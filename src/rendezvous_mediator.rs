...
async fn direct_server(server: ServerPtr) {
    ...
    loop {
        ...
        if !disabled && listener.is_none() {
            ...
            match claildesk_common::tcp::new_listener(&addr, false).await {
                Ok(l) => {
                    listener = Some(l);
                    log::info!(
        ...
        if let Some(l) = listener.as_mut() {
            ...
            if let Ok(Ok((stream, addr))) = claildesk_common::timeout(1000, l.accept()).await {
                ...
                tokio::spawn(async move {
                    allow_err!(
                        crate::server::create_tcp_connection(
                            server,
                            claildesk_common::Stream::from(stream, local_addr),
                            addr,
                            false,
                        )
                        .await
