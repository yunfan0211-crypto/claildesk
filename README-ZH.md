<p align="center">
  <img src="logo-header.svg" alt="ClailDesk - Your remote desktop"><br>
...
Chat with us: [知乎](https://www.zhihu.com/people/claildesk) | [Discord](https://discord.gg/nDceKgxnkV) | [Reddit](https://www.reddit.com/r/claildesk)

[![ko-fi](https://ko-fi.com/img/githubbutton_sm.svg)](https://ko-fi.com/I2I04VU09)

远程桌面软件，开箱即用，无需任何配置。您完全掌控数据，不用担心安全问题。您可以使用我们的注册/中继服务器，
或者[自己设置](https://claildesk.com/server)，
亦或者[开发您的版本](https://github.com/claildesk/claildesk-server-demo)。

欢迎大家贡献代码， 请看 [`CONTRIBUTING.md`](CONTRIBUTING.md).

[**可执行程序下载**](https://github.com/claildesk/claildesk/releases)
...
ClailDesk 暂时不支持 Wayland，不过正在积极开发中.
...
git clone https://github.com/claildesk/claildesk
cd claildesk
docker build -t "claildesk-builder" .
```
...
4. docker build 命令后面加上 proxy 参数
   ...
   ```
   docker build -t "claildesk-builder" . --build-arg http_proxy=http://host:port --build-arg https_proxy=http://host:port
   ```

然后，每次需要构建应用程序时，运行以下命令：

```sh
docker run --rm -it -v $PWD:/home/user/claildesk -v claildesk-git-cache:/home/user/.cargo/git -v claildesk-registry-cache:/home/user/.cargo/registry -e PUID="$(id -u)" -e PGID="$(id -g)" claildesk-builder
```
...
```sh
target/debug/claildesk
```

或者运行发布版本:

```sh
target/release/claildesk
```

请确保您从 ClailDesk 存储库的根目录运行这些命令，否则应用程序可能无法找到所需的资源。另请注意，此方法当前不支持其他`Cargo`子命令，
例如 `install` 或 `run`，因为运行在容器里，而不是宿主机上。

## 文件结构

- **[libs/hbb_common](https://github.com/claildesk/claildesk/tree/master/libs/hbb_common)**: 视频编解码, 配置, tcp/udp 封装, protobuf, 文件传输相关文件系统操作函数, 以及一些其他实用函数
- **[libs/scrap](https://github.com/claildesk/claildesk/tree/master/libs/scrap)**: 截屏
- **[libs/enigo](https://github.com/claildesk/claildesk/tree/master/libs/enigo)**: 平台相关的鼠标键盘输入
- **[src/ui](https://github.com/claildesk/claildesk/tree/master/src/ui)**: GUI
- **[src/server](https://github.com/claildesk/claildesk/tree/master/src/server)**: 被控端服务，audio/clipboard/input/video 服务, 已经连接实现
- **[src/client.rs](https://github.com/claildesk/claildesk/tree/master/src/client.rs)**: 控制端
- **[src/rendezvous_mediator.rs](https://github.com/claildesk/claildesk/tree/master/src/rendezvous_mediator.rs)**: 与[claildesk-server](https://github.com/claildesk/claildesk-server)保持 UDP 通讯, 等待远程连接（通过打洞直连或者中继）
- **[src/platform](https://github.com/claildesk/claildesk/tree/master/src/platform)**: 平台服务相关代码
