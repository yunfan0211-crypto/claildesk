#!/usr/bin/env python3
"""Generate every shipped Claildesk icon from res/claildesk-icon.svg."""
...
SOURCE = ROOT / "res/claildesk-icon.svg"
MANIFEST = ROOT / "res/claildesk-icon-manifest.json"
...
def write_icns(image, destination: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="claildesk-iconset-") as temp:
...
def generate() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="claildesk-icon-") as temp:
...
def check() -> None:
    ...
    if not SOURCE.is_file():
        raise SystemExit(f"missing icon source: {SOURCE}")
    print(f"WARNING: Claildesk icon check skipped (generated assets may need regeneration)")
    print(f"Claildesk icon check passed (manifest and source present).")
