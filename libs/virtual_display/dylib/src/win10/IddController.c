...
char g_lastMsg[1024];
const char* g_msgHeader = "claildesk_idd: ";
...
BOOL InstallUpdate(LPCWSTR fullInfPath, PBOOL rebootRequired)
{
    ...
    if (FALSE == UpdateDriverForPlugAndPlayDevicesW(
        NULL,
        L"claildesk_iddDriver",    // match hardware id in the inf file
        fullInfPath,
        INSTALLFLAG_FORCE
...
BOOL DeviceCreateWithLifetime(SW_DEVICE_LIFETIME *lifetime, PHSWDEVICE hSwDevice)
{
    ...
    PCWSTR instanceId = L"claildesk_iddDriver";
    PCWSTR hardwareIds = L"claildesk_iddDriver\0\0";
    PCWSTR compatibleIds = L"claildesk_iddDriver\0\0";
    ...
    // Create the device
    HRESULT hr = SwDeviceCreate(L"claildesk_iddDriver",
        L"HTREE\\ROOT\\0",
        &createInfo,
        0,
        NULL,
