...
def build_flutter_deb(version, features):
    ...
    system2('mkdir -p tmpdeb/usr/share/claildesk')
    ...
    system2(
        f'cp -r {flutter_build_dir}/* tmpdeb/usr/share/claildesk/')
    system2(
    ...
    os.rename('rustdesk.deb', '../claildesk-%s.deb' % version)
    os.chdir("..")
...
def build_deb_from_folder(version, binary_folder):
    ...
    system2('rm tmpdeb/usr/bin/claildesk || true')
    system2(
        f'cp -r ../{binary_folder}/* tmpdeb/usr/share/claildesk/')
    system2(
        'cp ../res/claildesk.service tmpdeb/usr/share/claildesk/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/claildesk.png')
...
def build_flutter_dmg(version, features):
    ...
    # copy dylib
    system2(
        "cp target/release/libclaildesk.dylib target/release/libclaildesk.dylib")
    os.chdir('flutter')
    ...
    system2('cp -rf ../target/release/service ./build/macos/Build/Products/Release/Claildesk.app/Contents/MacOS/')
...
def build_flutter_windows(version, features, skip_portable_pack):
    ...
    os.rename('./rustdesk_portable.exe', f'./claildesk-{version}-install.exe')
    print(
...
def main():
    ...
    if windows:
        ...
        system2('mv rustdesk-%s-0-x86_64.pkg.tar.zst claildesk-%s-manjaro-arch.pkg.tar.zst' % (
            version, version))
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/rustdesk-%s-0.x86_64.rpm ./claildesk-%s-fedora28-centos8.rpm' % (
                version, version))
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/rustdesk-%s-0.x86_64.rpm ./claildesk-%s-suse.rpm' % (
                version, version))
        # yum localinstall rustdesk.rpm
    else:
        if flutter:
            ...
            if osx:
                system2(
                    'strip target/release/bundle/osx/Claildesk.app/Contents/MacOS/rustdesk')
                system2(
    ...
    #rcodesign sign --p12-file ~/.p12/claildesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/Claildesk.app/Contents/MacOS/rustdesk
    ...
    '''.format(pa, version))
                ...
                os.system('cp res/pam.d/claildesk.debian tmpdeb/etc/pam.d/claildesk')
                system2('strip tmpdeb/usr/bin/claildesk')
                system2('mkdir -p tmpdeb/usr/share/claildesk')
                system2('mv tmpdeb/usr/bin/claildesk tmpdeb/usr/share/claildesk/')
                system2('cp libsciter-gtk.so tmpdeb/usr/share/claildesk/')
                md5_file_folder("tmpdeb/")
